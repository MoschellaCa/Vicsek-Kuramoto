...
function Vicsek_model!(du,u,p,t)
    @unpack R, v₀,L,k_1, k_2, N, system, τ  = p
    ...
    for i=1:N
        ...
        # dω_i/dt 
        du[i,4] = - u[i,4]*τ+ (k_2*1/N) * ((L^2)/(pi*(R^2))) * system.potential[i][2]
    end
    nothing
end
...
k_2=3 # factor multiplying the interaction potential in the angular velocity variable
end_time = 400 # Final time for the simulation
noise1 = 0.1
noise2 = 0.1
R = 1   #Interaction radius
v₀ = 3    #constant velocity of particles 
...
k_1_interval= [1, 50, 80]
k_2_interval= [50, 80]
τ_interval = [0.001, 0.1, 1, 10, 100]
noise_interval = [0.1, 1, 5]

for k_1 ∈ k_1_interval
    for k_2 ∈ k_2_interval
        for τ ∈ τ_interval
#            for noise1 ∈ noise_interval
#                for noise2 ∈ noise_interval

                    param = (R=R, v₀ , L, N, k_1, k_2, τ)
                    ...
                    param = (R=R, v₀, L, N, k_1, k_2, system, noise1, noise2, τ)
                    ...
                    plot!(size=(1000,800))
                    savefig(fig3,"Average_behavior_nonoise_N=$N,R=$(param[1])_v=$(param[2])_k1=$(param[5])_k2=$(param[6])_noise1=$(param[8])_noise2=$(param[9])_tau=$(param[10]).png") 
                    ...
                    gif(anim, "animationVicsek_nonoise_N=$N,R=$(param[1])_L=$(param[3])_v=$(param[2])_k1=$(param[5])_k2=$(param[6])_noise1=$(param[8])_noise2=$(param[9])_tau=$(param[10]).gif", fps = 10)
                    ...
                    savefig(fig5,"Average_trajectory_nonoise_N=$N,R=$(param[1])_L=$(param[3])_v=$(param[2])_k1=$(param[5])_k2=$(param[6])_noise1=$(param[8])_noise2=$(param[9])_tau=$(param[10]).png") 
