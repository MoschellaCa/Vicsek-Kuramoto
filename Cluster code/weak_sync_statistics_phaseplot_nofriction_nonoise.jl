...
function Vicsek_model!(du,u,p,t)
    ...
    for i=1:N
        ...
        # dω_i/dt 
        du[i,4] = + (k_2*1/N) * ((L^2)/(pi*(R^2))) * system.potential[i][2]
    end
    nothing
end
...
k_2=71 # factor multiplying the interaction potential in the angular velocity variable
...
param = (R=R, v₀, L, N, k_1, k_2, system, noise1, noise2)
...
plot!(size=(1000,800))
savefig(fig3,"Average_behavior_nofrictionnonoise_N=$N,R=$(param[1])_v=$(param[2])_k1=$(param[5])_k2=$(param[6])_noise1=$(param[8])_noise2=$(param[9]).png") 
...
gif(anim, "animationVicsek_nofrictionnonoise_N=$N,R=$(param[1])_L=$(param[3])_v=$(param[2])_k1=$(param[5])_k2=$(param[6])_noise1=$(param[8])_noise2=$(param[9]).gif", fps = 10)
...
savefig(fig5,"Average_trajectory_nofrictionnonoise_N=$N R=$(param[1])_L=$(param[3])_v=$(param[2])_k1=$(param[5])_k2=$(param[6])_noise1=$(param[8])_noise2=$(param[9]).png") 
