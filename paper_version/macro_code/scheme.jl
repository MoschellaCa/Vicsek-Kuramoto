...
One iteration of the numerical scheme which updates the data `(ρ,w,u,v)`. The scheme
...
function scheme_iter!(
    ...
    # U = (ρ,ρw,ρu,ρv), Ω = (u,v)
    # 1. (Compute the flux)
    # 2. (Conservative part) U^{n+1}_i = U^{n}_i - Δt/Δx(F^n_{i+1/2} - F^n_{i-1/2})
    # 3. (Relaxation) Ω^{n+1}_i = Ω^{n+1/2}_i/|Ω^{n+1/2}_i|
    # 4. (Boundary conditions)
    #*******************************************************************************#

    """ Remember that flux_x gives as an output the net flux ρΩ1,ρωΩ1, ρΩ1² + λρ, ρΩ1Ω2,
    so flux_ρ is the net flux difference ρΩ1 ecc..
    """
    #----------------------- 1. Compute the flux -----------------------------------#
    ...
    @inbounds for j in 2:(ncelly+1)
        @simd for i in 2:(ncellx+1)
            ...
            U = (ρ[i,j] - (Δt/Δy) * (flux_ρ[i-1, j] - flux_ρ[i-1, j-1]),
                 ρ[i,j]*w[i,j] - (Δt/Δy) * (flux_w[i-1, j] - flux_w[i-1, j-1]),
...
"""
function scheme_rotation!(ρ, w, u, v, Δt)
    ncellx = size(u)[1] - 2
    ncelly = size(u)[2] - 2
    @inbounds for j in 2:ncelly+1
        for i in 2:ncellx+1
            # Current angle:
            θ0 = atan(v[i,j], u[i,j])
            # Update angle by the local angular speed w[i,j]:
            θ  = θ0 + w[i,j] * Δt
