...
- `w` -- angular velocity (matrix of size `ncellx*ncelly`)
...
function run!(
    ...
    if should_plot || save_video
        ...
        if save_video
            fig1, ax1, hm1, arrows, stream1 = plot_rhoUV(ρ,w,u,v,Δx,Δy,range1,should_plot,save_video,plot_dir,"0.png";fps=fps,kwargs...)
            fig2, ax2, hm2, stream2 = plot_thetaUV(ρ,w,u,v,Δx,Δy,range2,should_plot,save_video,plot_dir,"0.png";fps=fps,kwargs...)

        else
            fig1, ax1, hm1, arrows = plot_rhoUV(ρ,w,u,v,Δx,Δy,range1,should_plot,save_video,plot_dir,"0.png";kwargs...)
            fig2, ax2, hm2 = plot_thetaUV(ρ,w,u,v,Δx,Δy,range2,should_plot,save_video,plot_dir,"0.png";kwargs...)
    ...
    flux_w = zeros(ncellx + 1, ncelly + 1)
    ...
    for itime in 1:ntime
        ...
        if should_plot || save_video
            if itime%plot_step == 0
                ...
                if save_video
                    update_plot!(fig1,ax1,hm1,arrows,ρ,w,u,v,"time=$round_time",plot_dir,"$itime.png",should_plot,stream1)
                    update_plot_theta!(fig2,ax2,hm2,ρ,w,u,v,"time=$round_time",plot_dir,"$itime.png",should_plot,stream2)

                else
                    update_plot!(fig1,ax1,hm1,arrows,ρ,w,u,v,"time=$round_time",plot_dir,"$itime.png")
                    update_plot_theta!(fig2,ax2,hm2,ρ,w,u,v,"time=$round_time",plot_dir,"$itime.png")
                end
            end
        end
        next!(p)
    ...
    if should_plot || save_video
        if save_video
            save(joinpath(dir_name,"video_rho.mp4"), stream1)
            save(joinpath(dir_name,"video_theta.mp4"), stream2)
            return fig2, ax2, hm2, stream2, fig1, ax1, hm1, stream1
        else
            return fig2, ax2, hm2, fig1, ax1, hm1 
        end
    end
