...
Sample a random data `(ρ,w,u,v)` of size `(ncellx+2)*(ncelly+2)`. The density `ρ` is sampled
...
function random_init(
    ...
    w = mean_w .+ range_w .* (rand(ncellx+2, ncelly+2) .- 0.5)
...
"""
PROBABLY NEED TO CHANGE THIS PART FOR THE SOURCE TERM
IN MY CASE THIS PART OF THE CODE IS NO LONGER NEEDED 
