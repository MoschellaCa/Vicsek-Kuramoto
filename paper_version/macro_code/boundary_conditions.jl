...
"""
    boundary_conditions_ρwuv!(ρ,u,v,bcond_x,bcond_y)

Apply the boundary conditions to the data `(ρ,w,u,v)`.
"""
function boundary_conditions_ρwuv!(ρ,w,u,v,bcond_x,bcond_y)
    if bcond_x == "periodic" || bcond_x == "Neumann"
        boundary_conditions_x!(ρ,bcond_x)
        boundary_conditions_x!(w,bcond_x)
        boundary_conditions_x!(u,bcond_x)
        boundary_conditions_x!(v,bcond_x)
    elseif bcond_x == "reflecting"
        boundary_conditions_x!(ρ,"Neumann")
        boundary_conditions_x!(w,"Neumann")      #or reflecting?
